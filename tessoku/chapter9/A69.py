...
def maxflow(N, s, t, edges):
	...
	G = [ list() for i in range(2*N + 3) ]
	for a, b, c in edges:
		G[a].append(maxflow_edge(b, c, len(G[b])))
		G[b].append(maxflow_edge(a, 0, len(G[a]) - 1))
	# 追加頂点
	for i in range(1, N+1):
		G[2*N+1].append(maxflow_edge(i, 1, len(G[i])))
		G[i].append(maxflow_edge(2*N+1, 0, len(G[2*N+1])-1))
		G[N+i].append(maxflow_edge(2*N+2, 1, len(G[2*N+2])))
		G[2*N+2].append(maxflow_edge(N+i, 0, len(G[N+i])-1))
	INF = 10 ** 10
	total_flow = 0
	while True:
...
# 答えを求めて出力
answer = maxflow(N, 2*N+1, 2*N+2, edges)
print(answer)