...
def beam_search(N, T, rounds):
	...
	beam[0].append(state(N)) # 0 手目の状態は1つのみ (0 点)
	...
	for i in range(T):
		...
		# 降順ソートして beam[i+1] の結果を計算する
		candidate.sort(key = lambda s: -s.score)
		beam[i + 1] = copy.deepcopy(candidate[:WIDTH]) # 点数の高い上位 WIDTH 件の candidate
	
	# ビームサーチの復元
	current_place = 0 # 一番点数の高い位置は常に 0 である
	answer = [ None ] * T
